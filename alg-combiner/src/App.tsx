  ...
  return (
    <main>
      <header className="app-header">
        <div>
          <h1>FTO Alg Combiner</h1>
          <p>By Abid Ibn Ashraf</p>
        </div>
        <div className="status-strip">
          <span className={`pill ${running ? "pill-running" : ""}`}>{status}</span>
          <span className="pill">{algNames.length} intermediate algs</span>
          <span className="pill">
            {counts.total.toLocaleString()} combos
          </span>
        </div>
      </header>
      ...
      <section className="workspace">
        ...
        <div className="control-pane">
          <section className="tool-section">
            ...
            <label className="field">
              <span>One per line: name : moves (# for comments)</span>
              <textarea
